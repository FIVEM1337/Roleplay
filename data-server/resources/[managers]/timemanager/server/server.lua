...
RegisterCommand(Config.Command.OpenUI, function(source)
    ...
    if PermissionsCheck(_source) then
        TriggerClientEvent('timemanager:OpenUI', _source, self)
    else
        Notification(_source, 3, L('invalid_permissions'))
    end
end)

RegisterServerEvent('timemanager:SyncMe')
AddEventHandler('timemanager:SyncMe', function(instant)
    local _source = source
    local temp = json.decode(json.encode(self))
    temp.instanttime = true
    temp.instantweather = true
    TriggerClientEvent('timemanager:ForceUpdate', _source, temp)
end)

RegisterServerEvent('timemanager:ForceUpdate')
AddEventHandler('timemanager:ForceUpdate', function(data)
    ...
    if PermissionsCheck(_source) then
        ...
        TriggerClientEvent('timemanager:ForceUpdate', -1, data)
    else
...
local function LoadSettings()
    ...
    if Config.Framework ~= 'vrp' or Config.Framework ~= 'aceperms' then
        ...
        temp.instanttime = true
        temp.instantweather = true
        TriggerClientEvent('timemanager:ForceUpdate', -1, temp)
    end
end
    ...
    while true do
        ...
        if not self.freeze then
            ...
            if not HasTriggered then
                HasTriggered = true
                TimeCounter = 0
                TriggerClientEvent('timemanager:SyncTime', -1, {hours = self.hours, mins = self.mins})
            end
        end
...
RegisterServerEvent('timemanager:ToggleInstantChange:Time')
AddEventHandler('timemanager:ToggleInstantChange:Time', function(boolean)
    self.instanttime = boolean
end)

RegisterServerEvent('timemanager:ToggleInstantChange:Weather')
AddEventHandler('timemanager:ToggleInstantChange:Weather', function(boolean)
...
AddEventHandler('timemanager:SaveSettings', function()
    SaveSettngs()
end)
...
AddEventHandler('txAdmin:events:scheduledRestart', function(eventData)
    if eventData.secondsRemaining == math.ceil(Config.TsunamiWarning_time*60) then
        ...
        self.tsunami = true
        TriggerClientEvent('timemanager:StartTsunamiCountdown', -1, true)
    end
end)

RegisterServerEvent('timemanager:StartTsunamiCountdown')
AddEventHandler('timemanager:StartTsunamiCountdown', function(boolean)
    if not Config.TsunamiWarning then return end
    self.tsunami = boolean
    TriggerClientEvent('timemanager:StartTsunamiCountdown', -1, boolean)
end)
