...
Citizen.CreateThread(function()
    if Config.Framework == 'esx' then
        ...
        AddEventHandler(Config.FrameworkTriggers.load, function(xPlayer)
            ...
            Wait(3000)
            TriggerServerEvent('timemanager:SyncMe', {time = true, weather = true})
        end)
...
RegisterNetEvent('timemanager:PauseSync')
AddEventHandler('timemanager:PauseSync', function(boolean, time)
    if boolean then
        ...
        PauseSync.state = false
        TriggerServerEvent('timemanager:SyncMe')
    end
end)

RegisterNetEvent('timemanager:ForceUpdate')
AddEventHandler('timemanager:ForceUpdate', function(data)
    ...
    if data.tsunami ~= nil and Config.TsunamiWarning and self.tsunami ~= data.tsunami then
        ...
        TriggerEvent('timemanager:StartTsunamiCountdown', data.tsunami)
    end
end)

RegisterNetEvent('timemanager:SyncWeather')
AddEventHandler('timemanager:SyncWeather', function(data)
    if not PauseSync.state then
        CheckSnowSync(data.weather)
        self.weather = data.weather
        ChangeWeather(self.weather, data.instantweather)
...
RegisterNetEvent('timemanager:SyncTime')
AddEventHandler('timemanager:SyncTime', function(data)
    if not PauseSync.state then
...
RegisterNUICallback('instanttime', function(data)
    TriggerServerEvent('timemanager:ToggleInstantChange:Time', data.instanttime)
end)

RegisterNUICallback('instantweather', function(data)
    TriggerServerEvent('timemanager:ToggleInstantChange:Weather', data.instantweather)
end)

RegisterNUICallback('change', function(data)
    NUI_status = false
    TriggerServerEvent('timemanager:ForceUpdate', data.values)
    if data.savesettings then
        print('Saving Settings - please wait 2 seconds ...')
        Wait(2000)
        TriggerServerEvent('timemanager:SaveSettings')
...
RegisterNetEvent('timemanager:OpenUI')
AddEventHandler('timemanager:OpenUI', function(values)
    TriggerEvent('timemanager:ToggleNUIFocus')
    SendNUIMessage({action = 'open', values = values})
end)

RegisterNetEvent('timemanager:ToggleNUIFocus')
AddEventHandler('timemanager:ToggleNUIFocus', function()
    NUI_status = true
    while NUI_status do
        Wait(5)
...
RegisterNetEvent('timemanager:StartTsunamiCountdown')
AddEventHandler('timemanager:StartTsunamiCountdown', function(boolean)
