...
InitFishing = function(cont)
    ...
    if StartFish == false then
        ...
        ESX.TriggerServerCallback("esx_fishing:getItems", function(cb)
            if cb == true then
                ...
                if waterValidated then
                    ...
                    StartFish = false
                    TriggerServerEvent("esx_fishing:BackFish")
                end
            else
			    --exports['mythic_notify']:SendAlert('error', 'You need a bait!')			   
                StartFish = false
            end
        end)
    end
        ...
        while #GLOBAL_FISH > 0 do
            ...
            for i=1, #GLOBAL_FISH, 1 do
                ...
                if Vdist(GLOBAL_COORDS.x, GLOBAL_COORDS.y, GLOBAL_COORDS.z, coords.x, coords.y, coords.z) <= 2.0 then
                    DrawText3Ds(coords.x, coords.y, coords.z, "[~g~E~w~] - Aufheben")
