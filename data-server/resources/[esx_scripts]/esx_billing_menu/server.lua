...
ESX.RegisterServerCallback("esx_billing_menu:GetInvoices", function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)

	MySQL.Async.fetchAll('SELECT * FROM rechnungen WHERE receiver_identifier = @identifier ORDER BY CASE WHEN status = "unpaid" THEN 1 WHEN status = "autopaid" THEN 2 WHEN status = "paid" THEN 3 WHEN status = "cancelled" THEN 4 END ASC, id DESC', {
...
RegisterServerEvent("esx_billing_menu:PayInvoice")
AddEventHandler("esx_billing_menu:PayInvoice", function(invoice_id)
	local xPlayer = ESX.GetPlayerFromId(source)

	MySQL.Async.fetchAll('SELECT * FROM rechnungen WHERE id = @id', {
		['@id'] = invoice_id
	}, function(result)
		...
		local vat = math.ceil(invoices.invoice_value * (Config.VATPercentage/100))
		local societies_value =  math.ceil(invoices.invoice_value - vat)
		...
		if playerMoney < invoices.invoice_value then
			TriggerClientEvent('dopeNotify:Alert', xPlayer.source, "", "Du hast nicht genug Geld!", 5000, 'error')	
		else
			...
			TriggerEvent("esx_addonaccount:getSharedAccount", invoices.society, function(account)
				if account ~= nil then
					account.addMoney(societies_value)
				end
			end)

			TriggerEvent("esx_addonaccount:getSharedAccount", "society_government", function(account)
				if account ~= nil then
					account.addMoney(vat)
				end
			end)

			MySQL.Async.execute('UPDATE rechnungen SET status = @status, paid_date = CURRENT_TIMESTAMP WHERE id = @id', {
				['@status'] = 'paid',
				['@id'] = invoice_id
			})

			TriggerClientEvent('dopeNotify:Alert', xPlayer.source, "", "Rechnung erfolgreich bezahlt!", 5000, 'success')
...
RegisterServerEvent("esx_billing_menu:CancelInvoice")
AddEventHandler("esx_billing_menu:CancelInvoice", function(invoice_id)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.fetchAll('SELECT * FROM rechnungen WHERE id = @id', {
		...
		MySQL.Async.execute('UPDATE rechnungen SET status = "cancelled", paid_date = CURRENT_TIMESTAMP WHERE id = @id', {
			['@id'] = invoice_id
		})
		TriggerClientEvent('dopeNotify:Alert', xPlayer.source, "", "Du hast die Rechnung storniert!", 5000, 'info')
		if Webhook ~= '' then
...
RegisterServerEvent("esx_billing_menu:CreateInvoice")
AddEventHandler("esx_billing_menu:CreateInvoice", function(data)
	local _source = ESX.GetPlayerFromId(source)
	local target = ESX.GetPlayerFromId(data.target)
	local webhookData = {}

	MySQL.Async.fetchAll('SELECT id FROM rechnungen WHERE id = (SELECT MAX(id) FROM rechnungen)', {}, function(result)
		webhookData = {
			id = result[1].id + 1,
	...
	if Config.LimitDate then
		MySQL.Async.insert('INSERT INTO rechnungen(receiver_identifier, receiver_name, author_identifier, author_name, society, society_name, item, invoice_value, status, notes, sent_date, limit_pay_date) VALUES (@receiver_identifier, @receiver_name, @author_identifier, @author_name, @society, @society_name, @item, @invoice_value, @status, @notes, CURRENT_TIMESTAMP(), DATE_ADD(CURRENT_TIMESTAMP(), INTERVAL @limit_pay_date DAY))', {
			...
			TriggerClientEvent('dopeNotify:Alert', target.source, "", "Du hast gerade eine neue Rechnung erhalten!", 5000, 'info')
			if Webhook ~= '' then
...
ESX.RegisterServerCallback("esx_billing_menu:GetSocietyInvoices", function(source, cb, society)
	local xPlayer = ESX.GetPlayerFromId(source)

	MySQL.Async.fetchAll('SELECT * FROM rechnungen WHERE society_name = @society ORDER BY id DESC', {
		['@society'] = society
	}, function(result)
		local invoices = {}
		...
		if result ~= nil then
			for i=1, #result, 1 do
				...
				if result[i].status == 'paid' then
					totalIncome = totalIncome + result[i].invoice_value - (ESX.Math.Round(result[i].invoice_value * (Config.VATPercentage/100)))
				elseif result[i].status == 'unpaid' then
					awaitedIncome = awaitedIncome + result[i].invoice_value - (ESX.Math.Round(result[i].invoice_value * (Config.VATPercentage/100)))
...
function checkTimeLeft()
	MySQL.Async.fetchAll('SELECT *, TIMESTAMPDIFF(HOUR, limit_pay_date, CURRENT_TIMESTAMP()) AS "timeLeft" FROM rechnungen WHERE status = @status', {
		['@status'] = 'unpaid'
	}, function(result)
				...
				for k, vl in pairs(whenToAddFees) do
					if v.fees_amount == k - 1 then
						if v.timeLeft >= vl*(-1) then
							MySQL.Async.execute('UPDATE rechnungen SET fees_amount = @fees_amount, invoice_value = @invoice_value WHERE id = @id', {
								['@fees_amount'] = k,
						...
						MySQL.Async.execute('UPDATE users SET accounts = @playerAccount WHERE identifier = @target', {
							...
							TriggerEvent("esx_addonaccount:getSharedAccount", v.society, function(account2)
								if account2 ~= nil then
									...
									MySQL.Async.execute('UPDATE rechnungen SET status = @paid, paid_date = CURRENT_TIMESTAMP() WHERE id = @id', {
										['@paid'] = 'autopaid',
										['@id'] = v.id
									})
								end
