...
Doors.Awake = function()
  ...
  Wait(500)
  TriggerServerEvent("esx_doorlock:Init")
...
Doors.MinigameCallback = function(door_index,door_state,item_name,did_win)
  if did_win then
    ...
    TriggerServerEvent("esx_doorlock:MinigameLocked",door_index,_s,item_name)
  else
...
Doors.BuyKey = function(data)
  ...
  TriggerServerEvent("esx_doorlock:PurchaseKey",data)
end
...
Doors.GetDoors = function()
  ...
  while true do
    ...
    if hit and ent > 0 then   
      ...
      if (ents[ent]) then
        ShowHelpNotification("~INPUT_AIM~ Fertig\n~INPUT_ATTACK~ Türe entfernen")
        ...
        if IsDisabledControlJustReleased(0,24) then
          if Doors.CheckDoor(ent) then
            ...
            ShowNotification("Die Türe ist bereits definiert.")
          end
        elseif IsDisabledControlJustReleased(0,25) then
          break
        end
...
Doors.GetTextLocation = function(doors)
  ...
  while true do
    ShowHelpNotification("[~g~ENTER~s~] Textposition festlegen")
...
Doors.CreationOptions = function(creation,callback)
  local elements = {
    {
      label = string.format("Verschlossen: %s",tostring( (creation.locked or false) )),
      value = (creation.locked or false),
      index = 'locked',
      prefix = "Locked",
    },
    {
      label = string.format("Neupositionieren: %s",tostring( (creation.reposition or false) )),
      value = (creation.reposition or false),
      index = 'reposition',
      prefix = "Reposition",
    },
    {
      label = string.format("Interagieren im Fahrzeug: %s",tostring( (creation.interact_in_veh or false) )),
      value = (creation.interact_in_veh or false),
      index = 'interact_in_veh',
      prefix = "Interact In Veh",
    },
    {
      label = string.format("Überfallen: %s",tostring( (creation.allow_raid or false) )),
      value = (creation.allow_raid or false),
      index = 'allow_raid',
      prefix = "Allow Raiding",
    },
    {
      label = string.format("Einbrechen: %s",tostring( (creation.can_break or false) )),
      value = (creation.can_break or false),
      index = 'can_break',
      prefix = "Allow Break In",
    },
    {
      label = string.format("Interaktionsdistanz: %s",tostring((creation.dist or 2.5))),
      value = (creation.dist or 2.5),
      index = 'dist',
      prefix = "Interact Dist",
    },
    {
      label = string.format("Renderdistanz: %s",tostring((creation.draw or 5.0))),
...
Doors.CreationJobs = function(creation,callback)
  ...
  ESX.TriggerServerCallback("esx_doorlock:GetJobs",function(jobs)
    local elements  = {}
    local _jobs     = {}
    ...
    table.insert(elements,{label = "Bestätigen",value = "Done"})

    ESX.UI.Menu.Open('default', GetCurrentResourceName(), "door_creation_jobs", {
        title    = "Autorisierte Jobs",
        align    = 'top-right',
        elements = elements,
      }, 
      function(data,menu)
    ...
    function(data,menu)
      ...
      if data.current.value == "done" then
        ...
        if callback then
          ...
          TriggerServerEvent("esx_doorlock:Save",creation)
        end
      else
...
Doors.Remove = function()
  ...
  if closest and closest_dist <= 10.0 then
    TriggerServerEvent("esx_doorlock:Remove",closest)
  else
    Doors.ShowNotification("Unable to find door nearby.")
  end
end

RegisterNetEvent("esx_doorlock:Remove")
AddEventHandler("esx_doorlock:Remove",Doors.Remove)

RegisterNetEvent("esx_doorlock:Removed")
AddEventHandler("esx_doorlock:Removed",Doors.Removed)

RegisterNetEvent("esx_doorlock:Init")
AddEventHandler("esx_doorlock:Init",Doors.Init)

RegisterNetEvent("esx_doorlock:SetLocked")
AddEventHandler("esx_doorlock:SetLocked",Doors.SetLocked)

RegisterNetEvent("esx_doorlock:NotifyPolice")
AddEventHandler("esx_doorlock:NotifyPolice",Doors.NotifyPolice)

RegisterNetEvent("esx_doorlock:Saved")
AddEventHandler("esx_doorlock:Saved",Doors.Saved)

RegisterNetEvent("esx_doorlock:CreateDoors")
AddEventHandler("esx_doorlock:CreateDoors",Doors.SetDoors)
