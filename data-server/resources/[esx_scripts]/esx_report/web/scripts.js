...
function viewFeedbackWindow(feedbackid) {
	...
	if (data.category === 'player_report') {
		feedbackWcard.find(".feedback_category").html(`<span><i class="fas fa-user-alt"></i> SPIELER MELDEN</span>`)
	} else if (data.category === 'question') {
		feedbackWcard.find(".feedback_category").html(`<span><i class="fas fa-question"></i> FRAGE</span>`)
	} else {
...
function reloadFeedbackTable() {
	...
	$.each(FeedbackTable, function(id, feedback) {
		...
		if (feedback.category === 'player_report') {
			category = "<span><i class='fas fa-user-alt'></i> SPIELER MELDEN</span>"
		} else if (feedback.category === 'question') {
			category = "<span><i class='fas fa-question'></i> FRAGE</span>"
		} else {
		...
		if (feedback.concluded != true) {
			feedbackTable.append(`
				<tr class="table-row" feedbackID="${id}">
					...
					<td><button type="button" class="btn btn-view"><i class="fas fa-eye"></i> BEARBEITEN</button></td>
				</tr>`
			)
		}
	});
	...
	if ($(".feedbackTable .table").children().length <= 0) {
		$(".feedbackTable .feedback_subtitle").html("AKTUELL LIEGEN KEINE SUPPORT ANFRAGEN VOR");
		...
		$(".feedback_subtitle").html("ES LIEGEN SUPPORT ANFRAGEN VOR");
