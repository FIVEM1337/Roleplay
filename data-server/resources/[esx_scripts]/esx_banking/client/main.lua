...
Citizen.CreateThread(function()
  ...
  while true do
    ...
    if nearBankorATM() then
      _sleep = false
      DisplayHelpText("Drücke ~INPUT_PICKUP~ um die Bank zu eröffnen ~b~")
      if IsControlJustPressed(1, 38) then
        inMenu = true
        SetNuiFocus(true, true)
        SendNUIMessage({type = 'openGeneral', banco = atbank})
        TriggerServerEvent('esx_banking:server:balance', inMenu)
      end
...
RegisterNetEvent('esx_banking:client:refreshbalance')
AddEventHandler('esx_banking:client:refreshbalance', function(balance)
  ...
  ESX.TriggerServerCallback('esx_banking:server:GetBankData', function(playerName, cardnumber1, cardnumber2, cardnumber3, cardnumber4)
    SendNUIMessage({
      type = "balanceHUD",
      balance = balance,
      player = playerName,
...
RegisterNUICallback('deposit', function(data)
	TriggerServerEvent('esx_banking:server:depositvb', tonumber(data.amount), inMenu)
	TriggerServerEvent('esx_banking:server:balance', inMenu)
end)

RegisterNUICallback('withdraw', function(data)
	TriggerServerEvent('esx_banking:server:withdrawvb', tonumber(data.amountw), inMenu)
	TriggerServerEvent('esx_banking:server:balance', inMenu)
end)
...
RegisterNUICallback('transfer', function(data)
	TriggerServerEvent('esx_banking:server:transfervb', data.to, data.amountt, inMenu)
	TriggerServerEvent('esx_banking:server:balance', inMenu)
end)

RegisterNetEvent('esx_banking:result')
AddEventHandler('esx_banking:result', function(type, message)
