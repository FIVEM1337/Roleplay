...
if lang == nil then
    print('^5[CryptoHooker] ^1Code: Could not load language file. Make sure you didn\'t make a typo.^0')
    return StopResource(GetCurrentResourceName())
end

if cfgFile == nil then
    print('^5[CryptoHooker] ^1Code: Could not load config file. Make sure you didn\'t make a typo.^0')
    return StopResource(GetCurrentResourceName())
end

RegisterNetEvent('CryptoHooker:Debug')
AddEventHandler('CryptoHooker:Debug', log)

RegisterNetEvent('CryptoHooker:errorLog')
AddEventHandler('CryptoHooker:errorLog', errorLog)

function debugLog(x)
	if JD_Debug then
		print("^5[CryptoHooker]^0 " .. x)
	end
end

function errorLog(x)
	print("^5[CryptoHooker]^1 " .. x .."^0")
end

RegisterNetEvent("CryptoHooker:discordLogs")
AddEventHandler("CryptoHooker:discordLogs", function(message, color, channel)
    discordLog(message, color, channel)
end)

RegisterNetEvent("CryptoHooker:ClientUploadScreenshot")
AddEventHandler("CryptoHooker:ClientUploadScreenshot", function(args)
	...
	if args.screenshot then
		if GetResourceState('screenshot-basic') == "started" then
			...
			if webhooksFile['imageStore'].webhook ~= "" and webhooksFile['imageStore'].webhook ~= "DISCORD_WEBHOOK" then
				...
				TriggerClientEvent('CryptoHooker:ClientCreateScreenshot', args.player_id, args)
			else
...
RegisterServerEvent('CryptoHooker:playerDied')
AddEventHandler('CryptoHooker:playerDied',function(args)
...
RegisterServerEvent('CryptoHooker:playerShotWeapon')
AddEventHandler('CryptoHooker:playerShotWeapon', function(weapon, count)
...
RegisterServerEvent('CryptoHooker:ClientDiscord')
AddEventHandler('CryptoHooker:ClientDiscord', function(args)
...
local storage = nil
RegisterNetEvent('CryptoHooker:sendClientLogStorage')
AddEventHandler('CryptoHooker:sendClientLogStorage', function(_storage)
	storage = _storage
end)
...
Commands.logs = function(source, args, RawCommand)
	if IsPlayerAceAllowed(source, cfgFile.logHistoryPerms) then
		if tonumber(args[1]) then
			...
			TriggerClientEvent('CryptoHooker:getClientLogStorage', args[1])
			Wait(500)
	...
	if source == 0 then
		if args[1] and has_val(GetPlayers(), args[1]) then
			if GetResourceState('screenshot-basic') == "started" then
				...
				TriggerClientEvent('CryptoHooker:ClientCreateScreenshot', args[1], args)
...
RegisterServerEvent('CryptoHooker:eventLoggerClient')
AddEventHandler('CryptoHooker:eventLoggerClient', function(args)
	ServerFunc.CreateLog(args)
end)

if GetCurrentResourceName() ~= "CryptoHooker" then
    errorLog('This recource should be named "CryptoHooker" for the exports to work properly.')
end
