...
if cfgFile['EnableAcFunctions'] then
    RegisterNetEvent('CryptoHooker:DropPlayer')
    AddEventHandler('CryptoHooker:DropPlayer', function(reason)
        if not IsPlayerAceAllowed(source, cfgFile['AntiCheatBypass']) then
            DropPlayer(source, 'Automated kick: '..reason)
        end
    end)

    RegisterNetEvent('CryptoHooker:getACConfig')
    AddEventHandler('CryptoHooker:getACConfig', function()
        TriggerClientEvent('CryptoHooker:SendACConfig', source, ACcfgFile)
    end)
    ...
    RegisterNetEvent("CryptoHooker:resourceCheck")
    AddEventHandler("CryptoHooker:resourceCheck", function(rcList)
        local source = source
