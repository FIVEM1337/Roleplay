...
if webhooksFile == nil then
    print('^5[CryptoHooker] ^1Error: Could not load webhooks file. Make sure you didn\'t make a typo.^0')
...
function GetPlayerDetails(src, config, channel)
    ...
    if GetConvar("steam_webApiKey", "false") ~= 'false' then
        ...
        _steamID = ""
        _steamURL = ""
        TriggerEvent('CryptoHooker:errorLog', 'You need to set a steam api key in your server.cfg for the steam identifiers to work!.')
    end
    ...
    if config.Session or config.PlayTime and not webhooksFile[channel].Hide['PlayTime'] then
        if GetResourceState('Prefech_playTime') == "started" then
            ...
            TriggerEvent('CryptoHooker:errorLog', 'Prefech_playTime is not installed.')
            _total = ""
            _session = ""
        end
    else
    ...
    if config['playerHealth'] or config['playerArmor'] then
        ...
        if config['playerHealth'] and not webhooksFile[channel].Hide['playerHealth'] then
            ...
            _hp = _hp.."**Leben:** ❤: `"..health.."/"..maxHealth.."`"
        end
        if config['playerArmor'] and not webhooksFile[channel].Hide['playerArmor'] then
            ...
            _hp = _hp.."**Rüstung:** 🛡: `"..armour.."/"..maxArmour.."`"
        end
    else
        _hp = ""
    end
    ...
    if webhooksFile[args.channel] then
        ...
        if args.player_id then
            if webhooksFile[args.channel].logHistory then
                TriggerClientEvent('CryptoHooker:ClientLogStorage', args.player_id, { Channel = args.channel, Message = args.EmbedMessage, TimeStamp = os.date("%x %X %p") })
            end
            ...
            if GetPlayerName(args.player_id) ~= nil then
                message['embeds'][1].fields = {
                    {
                        ["name"] = "Spieler: "..GetPlayerName(args.player_id),
                ...
                message['embeds'][1].fields = {
                    {
                        ["name"] = "Spieler: Unbekannt",
                        ["value"] = "Unbekannt - Die Informationen des Spielers konnten nicht abgerufen werden.",
                        ["inline"] = configFile.inlineField
                    }
                }
            end
        ...
        if args.player_2_id then
            if webhooksFile[args.channel].logHistory then
                TriggerClientEvent('CryptoHooker:ClientLogStorage', args.player_2_id, { Channel = args.channel, Message = args.EmbedMessage, TimeStamp = os.date("%x %X %p") })
            end
            Player_2_Details = GetPlayerDetails(args.player_2_id, configFile, args.channel)
            message['embeds'][1].fields[2]  = {
                ["name"] = "Spieler: "..GetPlayerName(args.player_2_id),
        ...
        print("CryptoHooker: Webhooks channel not found. ("..args.channel..")")
    end
end
...
function getStatus(status, channel)
	if status == 404 or status == 401 and webhooksFile[channel].webhook ~= "DISCORD_WEBHOOK" and webhooksFile[channel].webhook ~= "" then
		print('^3Warn: CryptoHooker webhook. Possible invalid webhook for "'..channel..'" webhook. Status code: '..status)
	end
end
