...
if lang == nil then
    print('^5[CryptoHooker] ^1Error: Could not load language file. Make sure you didn\'t make a typo.^0')
    return StopResource(GetCurrentResourceName())
end

if cfgFile == nil then
    print('^5[CryptoHooker] ^1Error: Could not load config file. Make sure you didn\'t make a typo.^0')
    return StopResource(GetCurrentResourceName())
end
	...
	while true do
		...
		if IsEntityDead(PlayerPedId()) then
			...
			if DeathReason == lang['DeathReasons'].Suicide or DeathReason == lang['DeathReasons'].Died then
				TriggerServerEvent('CryptoHooker:playerDied', { type = 1, player_id = GetPlayerServerId(PlayerId()), death_reason = DeathReason, weapon = Weapon })
			else
				TriggerServerEvent('CryptoHooker:playerDied', { type = 2, player_id = GetPlayerServerId(PlayerId()), player_2_id = GetPlayerServerId(Killer), death_reason = DeathReason, weapon = Weapon })
			end
			Killer = nil
			DeathReason = nil
			DeathCauseHash = nil
...
RegisterNetEvent('CryptoHooker:ClientCreateScreenshot')
AddEventHandler('CryptoHooker:ClientCreateScreenshot', function(args)
	if args.url ~= "" and args.url ~= nil and args.url ~= 'DISCORD_WEBHOOK' then
		exports['screenshot-basic']:requestScreenshotUpload(args.url, 'files[]', function(data)
			...
			if resp.attachments then
				...
				TriggerServerEvent('CryptoHooker:ClientUploadScreenshot', args)
			else
				TriggerServerEvent('CryptoHooker:Debug', 'Screenshot Failed.', "Make sure you have setup screenshot-basic correctly.")
			end
		end)
	else
		TriggerServerEvent('CryptoHooker:Debug', 'Screenshot Failed.', "No webhook url found in imageStore.")
	end
end)
		...
		elseif not IsPedShooting(playerped) and fireCount ~= 0 and timeout ~= 0 then
			...
			if fireCount ~= 0 and timeout == 0 then
				if not ClientWeapons.WeaponNames[tostring(fireWeapon)] then
					TriggerServerEvent('CryptoHooker:playerShotWeapon', lang['WeaponFired'].Undefined)
					TriggerServerEvent('CryptoHooker:Debug', 'Weapon not defined.', "Weapon not listed: "..tostring(fireWeapon))
					return
				end
				...
				if isLoggedWeapon then
					TriggerServerEvent('CryptoHooker:playerShotWeapon', ClientWeapons.WeaponNames[tostring(fireWeapon)], fireCount)
				end
				fireCount = 0
			end
		end
...
exports('discord', function(message, id, id2, color, channel)
	...
	TriggerServerEvent('CryptoHooker:ClientDiscord', args)
	local resource = GetInvokingResource()
	TriggerServerEvent('CryptoHooker:Debug', 'Server Old Export from '..resource)
end)

exports('createLog', function(args)
	TriggerServerEvent('CryptoHooker:ClientDiscord', args)
	local resource = GetInvokingResource()
	TriggerServerEvent('CryptoHooker:Debug', 'Server New Export from '..resource)
end)

local clientStorage = {}
RegisterNetEvent('CryptoHooker:ClientLogStorage')
AddEventHandler('CryptoHooker:ClientLogStorage', function(args)
...
RegisterNetEvent('CryptoHooker:getClientLogStorage')
AddEventHandler('CryptoHooker:getClientLogStorage', function()
    TriggerServerEvent('CryptoHooker:sendClientLogStorage', clientStorage)
end)
...
if type(eventsFile) == "table" then
	for k,v in pairs(eventsFile) do
		if not v.Server then
			TriggerServerEvent('CryptoHooker:Debug', 'Added Client Event Log: '..v.Event)
			AddEventHandler(v.Event, function()
				TriggerServerEvent('CryptoHooker:ClientDiscord', {EmbedMessage = 'EventLogger: '..v.Message, channel = v.Channel})
				TriggerServerEvent('CryptoHooker:eventLoggerClient', {EmbedMessage = 'EventLogger: '..v.Message, channel = v.Channel})
			end)
		end
	end
end