...
if cfgFile['EnableAcFunctions'] then
    ...
    CreateThread(function()
        TriggerServerEvent('CryptoHooker:getACConfig')
        while true do
            Wait(60 * 1000)
            TriggerServerEvent('CryptoHooker:getACConfig')
        end
	...
	local acConfig = {}
	RegisterNetEvent('CryptoHooker:SendACConfig')
	AddEventHandler('CryptoHooker:SendACConfig', function(_config)
		acConfig = _config
	end)
				...
				for k, v in pairs(acConfig['BlacklistedVehicles']) do
					if (IsVehicleModel(vehicle, v)) then
						...
						if GetPlayerServerId(PlayerId()) ~= nil then
							TriggerServerEvent('CryptoHooker:ClientDiscord', {EmbedMessage = lang['AntiCheat'].ACBlacklistedVehLog:format(GetPlayerName(PlayerId()), v), player_id = GetPlayerServerId(PlayerId()), channel = 'AntiCheat'})
						end
						...
						if acConfig['KickSettings'].BlacklistedVehicles then
							if warnLimit == acConfig['KickSettings'].BlacklistedVehicleLimit then
								...
								Wait(100)
								TriggerServerEvent('CryptoHooker:DropPlayer', lang['AntiCheat'].ACBlacklistedVehKick)
							end
						end
					end
				end
			...
			while not finished do
				...
				for k,v in pairs(acConfig['BlacklistedObjects']) do
					if (GetEntityModel(object) == GetHashKey(v)) then
						DeleteObject(object)
						if GetPlayerServerId(PlayerId()) ~= nil then
							TriggerServerEvent('CryptoHooker:ClientDiscord', {EmbedMessage = lang['AntiCheat'].BlacklistedObject:format(v), player_id = GetPlayerServerId(PlayerId()), channel = 'AntiCheat'})
		...
		while true do
			...
			for k,v in pairs(acConfig['BlacklistedKeys']) do
				if IsControlJustReleased(0, tonumber(k)) and not IsNuiFocused() then
					Wait(500)
					if GetPlayerServerId(PlayerId()) ~= nil then
						TriggerServerEvent('CryptoHooker:ClientDiscord', {EmbedMessage = lang['AntiCheat'].BlacklistedKey:format(k, v), player_id = GetPlayerServerId(PlayerId()), channel = 'AntiCheat',})
	...
	CreateThread(function()
		while true do
			...
			Wait(5000)
			TriggerServerEvent("CryptoHooker:resourceCheck", resourceList)
		end
	end)
				...
				for k,x in pairs(acConfig['BlacklistedCommands']) do
					for k,z in pairs(prefixCheck) do
						if string.lower(v.name) == string.lower(z..""..x) then
							if GetPlayerServerId(PlayerId()) ~= nil then
								TriggerServerEvent('CryptoHooker:ClientDiscord', {EmbedMessage = lang['AntiCheat'].BlacklistedCommand:format(x), player_id = GetPlayerServerId(PlayerId()), channel = 'AntiCheat'})
							end
							...
							if acConfig['KickSettings'].BlacklistedCommands then
								TriggerServerEvent('CryptoHooker:DropPlayer', lang['AntiCheat'].BlacklistedCommandKick)
							end
						end
					end
				end
